...
Die Motoren werden jeweils über das Molex Micro-Fit Stecksystem auf die MainPCB Platine aufgesteckt und werden anschließend mittels 3D-Druck Halterung an die Platine angeschraubt.
...
\end{figure}
Wie die Illustation \ref{fig:esp32powerup} zeigt, muss der Enable Pin (im Datenblatt \cite{ESP32} CHIP\_PU genannt) für eine bestimmte Zeit $t_0 = 50\mu s$ nach dem Power-Up auf Low gehalten werden.
...
Geschwindigkeit und Drehrichtung der Motoren wird über die Pulseweite der (AIN1/AIN2; BIN1/BIN2) 
...
Außerdem wird der Eingang BIN mit DO der vorletzten LED verbunden und bringt somit Redundanz beim Aufall einer LED.
...
Da mit 5V und 3.3V kein großer Verbraucher gespeist wird, ist der Baustein mit maximal 3A Dauerstrom ausreichend dimensioniert.
...
\begin{description}
    ...
    Anschlie{\ss}end muss REGIN mit VDD verbunden werden (Lötbrücke blau) und mithilfe von z.B. einem Kupferdraht mit 3.3V versorgt werden. (Abbildung \ref{fig:cp2102nfehlerkorrekturlayout} blau)
\end{description}
