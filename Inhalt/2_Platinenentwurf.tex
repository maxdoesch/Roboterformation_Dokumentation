...
Die Motoren werden jeweils über das Molex Micro-Fit Stecksystem auf die MainPCB Platine aufgesteckt und werden anschlie{\ss}end mittels 3D-Druck Halterung an die Platine angeschraubt.
...
Das Marvelmind benötigt 5V. Diese werden auf der LED\_PCB Platine erzeugt.
...
\end{figure}
Wie die Illustration \ref{fig:esp32powerup} zeigt, muss der Enable Pin (im Datenblatt \cite{ESP32} CHIP\_PU genannt) für eine bestimmte Zeit $t_0 = 50\mu s$ nach dem Power-Up auf Low gehalten werden.
...
Diese ermöglicht zum Beispiel das Schreiben in den Flash-Speicher im Dowload-Boot und einen seriellen Monitor für z.B. printf Debugging im SPI-Boot.
...
Der SS8050 Baustein ist ein npn-Bipolartransistoren. Diese Schaltung ist parallel geschaltet zu der Möglichkeit ESP\_EN und ESP\_IO0 über einen Taster auf GND zu ziehen.
...
\begin{description}
    ...
    Anschlie{\ss}end muss REGIN mit VDD verbunden werden (Lötbrücke blau) und mithilfe beispielsweise eines Kupferdrahtes mit 3.3V versorgt werden. (Abbildung \ref{fig:cp2102nfehlerkorrekturlayout} blau)
\end{description}
