...
\begin{figure}[h]
    ...
    \caption{Blockschaltbild PCB}
    \label{fig:blockschaltbild_pcb}
\end{figure}
Das Blockschaltbild \ref{fig:blockschaltbild_pcb} zeigt den Leistungs- und Signalfluss zwischen den Bauteilen auf beiden Platinen.
Die primäre Stromversorgung übernehmen drei 18650 LiIon Zellen. Diese haben im geladenen Zustand eine Gesamtspannung von $3\cdot4.2V = 12.6V$ 
und können über einen Schalter an- und ausgeschalten werden. Der Roboter ist gegenüber Verpolung aller Zellen geschützt.
Für die Versorgung des Mikrocontrollers müssen die 12V mit einem Buck-Converter auf 3.3V gewandelt werden.
Au{\ss}erdem soll der Roboter für den Test-/Programmierbetrieb auch über die 5V der USB-C oder der JTAG/Programmierschnittstelle stationär betrieben werden können.
...
Die Motoren werden jeweils über das Molex Micro-Fit Stecksystem auf die MainPCB Platine aufgesteckt und anschlie{\ss}end mittels 3D-Druck Halterung an die Platine angeschraubt.
...
Die beiden externen Marvelmind Mikrofone können einfach auf die LED\_PCB Platine gelötet werden. 
Das Marvelmind Modul benötigt 5V. Diese werden auf der LED\_PCB Platine erzeugt.
Die 43 RGB Led-ICs werden über die 12V Spannung versorgt und mittels einer Datenleitung vom Mikrocontroller angesteuert.
Da die Led Bausteine 5V Logikpegel benötigen, wird das 3.3V Signal vom ESP32 mittels eines 1-Bit Level Shifters verstärkt.
...
und 8MB externen SRAM, sowie 8MB externen Flash Speicher. \cite{ESP32WROVER-E}
...
\end{figure}
Damit der ESP32 erfolgreich startet, muss der Enable Pin (im Datenblatt \cite{ESP32} CHIP\_PU genannt) für eine bestimmte Zeit $t_0 = 50\mu s$ nach dem Power-Up auf Low gehalten werden.
Das Gleiche gilt für den Neustart des Mikrocontrollers $t_1 = 50\mu s$. Illustration \ref{fig:esp32powerup} veranschaulicht das Timing zwischen Stromversorgung VDD und CHIP\_PU. 
Die Power-Up/Reset Schaltung könnte zum Beispiel als Parallelschaltung eines RC Tiefpasses mit einem Taster umgesetzt werden.
...
Der Download Boot dient zum Flashen einer neuen Firmware. Für den Wechsel zwischen den Modis wertet der Mikrocontroller beim Start die Strapping Pins GPIO0 und GPIO2 aus.
\begin{table}[H]
    \centering
...
\end{table}
Da GPIO2 intern auf LOW gezogen wird, muss dieser Pin nicht verbunden werden. Die Boot Modus Auswahl geschieht dann alleine über GPIO0.

\subsubsection*{UART0}
Über die Pins UART0-RX/TX wird eine serielle Verbindung hergestellt. 
Diese ermöglicht zum Beispiel das Schreiben in den Flash-Speicher im Dowload-Boot. 
Außerdem ist über einen seriellen Monitor z.B. printf Debugging im SPI-Boot möglich.

\subsubsection*{JTAG}
TMS, TDI, TCK, TDO können für das JTAG-Debugging verwendet werden. 

\subsubsection*{I2C}
Auf beiden Platinen sind Anschlüsse an den I2C-Bus des ESP32 vorgesehen. Über diesen soll primär eine 9DOF-IMU (Intertial Measurement Unit) betrieben werden.

\subsection{CP2102N}
\label{sub:cp2102n}
Der Chip CP2102N \cite{CP2102N} wirkt als "'Übersetzer"' zwischen der USB2.0 Fullspeed Schnittstelle und dem UART Interface.
...
Dies ermöglicht das Programmieren des ESP32 einfach über USB. Au{\ss}erdem kann auch auf den seriellen Monitor zugegriffen werden.
Des Weiteren kann das RTS (Ready to Send; LOW aktiv) und DTR (Data Terminal Ready; LOW aktiv) Signal zur Auswahl des Boot Modus und Neustarten des ESP32 Mikrocontrollers verwendet werden.
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
Mit der doppel H-Brücke DRV8841 \cite{DRV8841} von Texas Instruments können zwei DC-Motoren im 2 Quadranten-Betrieb betrieben werden.
...
Geschwindigkeit und Drehrichtung der Motoren wird über die Pulsweite der (AIN1/AIN2; BIN1/BIN2) 
Eingangssignale gesteuert. Abbildung \ref{tab:drv8841_hbridgelogic} zeigt die logische Verknüfung der Eingangs- und Ausgangssignale der H-Brücke.
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
Der Marvelmind Mini-Rx Beacon \cite{Marvelmind} empfängt die Ultraschallsignale der HW4.9 Beacons für die Berechnung der Pose des Roboters.
Über die integrierte UART Schnittstelle wird die Pose an den Mikrocontroller gesendet.
Da das Modul eine dritte Platine ist, wurde darauf geachtet diese möglichst platzschonend in den Platinenstack zu integrieren.
Hierfür musste das Gehäuse und der eingebaute Akku entfernt werden. Die 5V Stromversorgung wird von der LED Platine gewährleistet.
...
Da die Marvelmind UART Schnittstelle mit 3.3V Logikpegeln arbeitet, kann diese ohne Probleme direkt mit dem ESP32 verbunden werden.
Des Weiteren wurde das eingebaute Mikrofon durch zwei Externe ersetzt. Diese sind über das 2.0mm 2x4 Pinout mit dem Mini-RX verbunden \ref{fig:marvelmind_micpinout}.
...
\end{figure}
Der Chip WS2815B ist eine RGB-Led mit integriertem LED-Treiber. Dieser Baustein kann über die Pins DO, DIN in Reihe zu einem Led-Array verschalten werden. 
...
Des Weiteren wird 1 Bit nicht über den Wert der Spannungspegel definiert, sondern in die Dauer der High und Low-Phasen codiert.
...
Au{\ss}erdem wird der Eingang BIN mit DO der vorletzten LED verbunden und bringt somit Redundanz beim Ausfall einer LED.
...
Der Chip PI4ULS5V201 \cite{PI4ULS5V201} ist ein Level-Shifter IC und kann den 3.3V Pegel des ESP32 auf die benötigten 5V verstärken. 
...
\end{figure}
Auf den beiden Platinen existieren insgesamt drei verschiedene Spannungen (12V, 5V, 3.3V). 
Mit dem Buck Converter TPS54331 \cite{TPS54331} werden jeweils 5V und 3.3V aus 12V erzeugt.
Da mit 5V und 3.3V kein gro{\ss}er Verbraucher gespeist wird, ist der Baustein mit maximal 3A Dauerstrom ausreichend dimensioniert.
...
Das Datenblatt liefert eine Tabelle zur Dimensionierung aller passiven Bauelemente je nach benötigter Eingangs- und Ausgangsspannung.
...
Es beinhaltet unter anderem das Tool "'Eeschema"' ein Schaltplan Editor und "'PCBNew"' zur Erstellung des PCB Layouts.
Der Schaltplan wurde nach den Vorgaben und Applikationsbeispielen in den Datenblättern der Bauelemente erstellt.
Aus diesem Grund wird im Folgenden nur auf Besonderheiten im Schaltplan eingegangen.
Die vollständigen Schaltpläne für beide Platinen befinden sich im Anhang \ref{ch:mainpcb} und Anhang \ref{ch:ledpcb} am Ende dieser Dokumentation.
...
Au{\ss}erdem wird die Platine vor Verpolung der LiIon Zellen und Überstrom/Kurzschluss geschützt.
...
$V_{GS} \approx -12V$. Bei Verpolung hat das Transistor Gate das Potential 12V. $V_{GS} < 0V$ ist somit nicht möglich und der Mosfet sperrt.
...
da sonst die Gefahr eines Kurzschlusses zwischen den Potentialen bestünde, wenn mehrere Spannungsquellen gleichzeitig aktiv sind.
VBUS und 5VD werden durch Dioden vor Potentialausgleich geschützt. Diese Spannungen werden nur zum Debuggen verwendet. 
Es flie{\ss}t nur wenig Strom und der Spannungsabfall über den Dioden ist somit hinnehmbar. 
Da die 12V den Roboter im Normalbetrieb mit teilweise gro{\ss}er Stromaufnahme versorgen, darf es keinen gro{\ss}en Spannungsabfall über der schützenden Komponente geben.
Der PMOS Transistor AO3401 hat einen geringen Drain-Source Widerstand und somit wenig Leistungsverlust bei gro{\ss}em Strom.
...
\end{figure}
In Kapitel \ref{sub:cp2102n} wurde gezeigt, dass es möglich ist den ESP32 über das DTR und RTS Signal des CP2102N neuzustarten und den Bootmodus zu wechseln.
...
Der SS8050 Baustein ist ein npn-Bipolartransistor. Diese Schaltung ist parallel geschaltet zu der Möglichkeit ESP\_EN und ESP\_IO0 über einen Taster auf GND zu ziehen.

\section{Layout}
Das Layout wurde mit dem Tool PCBNew aus KiCad erstellt. 
Die MainPCB Platine \ref{fig:kicad_mainpcb_layout} besitzt vier Kupferschichten (1. Schicht: Signal, 2.: 3.3V, 3.: 12V, 4.: GND) und das LedPCB \ref{fig:kicad_ledpcb_layout} besitzt zwei Kupferschichten (1.: 12V/Signal, 2.: GND).
Bei der Erstellung wurde versucht Angaben in den Datenblättern zu befolgen.
Im Folgenden wird deshalb nicht weiter darauf eingegangen.
\begin{figure}[H]
    \centering
...
Der Schaltplan enthält drei Fehler. Diese können allerdings ausgebessert werden oder beinträchtigen die Nutzung der Platine nur gering.
Der Dokumentation liegen einmal das fehlerhafte KiCad Projekt V1.0 und ein Projekt V1.1 mit ausgebesserten Fehlern bei.
\subsection*{3.3V Spannungswandler}
\begin{description}
    \item[Fehlerbeschreibung:] Der 3.3V Spannungwandler auf der MainPCB Platine schaltet bei einer Eingangsspannung von 5V (USB, JTAG-/Programmierinterface) ab. 
    Die Spannungswandlung von 12V zu 3.3V funktioniert. 
    ...
    Diese Widerstände wurden für 5V falsch dimensioniert.
...
\begin{description}
    \item[Fehlerbeschreibung:] Der Mikrocontroller startet nicht richtig, wenn die Platine über 12V versorgt wird und USB-C nicht verbunden ist.
    ...
    \item[Fehlerbehebung:] Damit die Ausgänge RTS und DTR immer einen definiertem Pegel haben, muss der CP2102N dauerhaft über die 3.3V des Spannungswandlers versorgt sein. 
    Dazu muss REGIN mit 3.3V verbunden werden. Abbildung \ref{fig:cp2102nfehlerkorrektur} zeigt, dass die Eingänge mit dem Label \bverb{CP2102N_VDD} direkt mit 3.3V verbunden werden können.
    Auf den bestehenden Platinen kann der Fehler behoben werden, indem der CP2102N Chip abgelötet wird und die Leiterspur zwischen VBUS und REGIN aufgetrennt wird. (Abbildung \ref{fig:cp2102nfehlerkorrekturlayout} grün)
    Anschlie{\ss}end muss REGIN mit VDD verbunden werden (Lötbrücke blau) und mit Hilfe eines Kupferdrahtes mit 3.3V versorgt werden. (Abbildung \ref{fig:cp2102nfehlerkorrekturlayout} blau)
\end{description}
...
Zukünftige Projektarbeiten werden die Platine weiter entwickeln.
Im Folgenden sollen deshalb mögliche Verbesserungsvorschläge aufgelistet werden.
\begin{itemize}
    \item Da das Ziel des Gesamtprojektes eine Roboterformation mit vielen einzelnen Robotern ist, muss zukünftig die Ladeinfrastruktur einfach gehalten werden.
    ...
    Diese könnte auf einen zweiten Mikrocontroller ausgelagert werden. Somit würden wieder mehr Pins am ESP32 nutzbar sein. 
    Au{\ss}erdem wird die ESP32 Software-Architektur vereinfacht und Rechenleistung für zukünftige Software Erweiterungen frei.
\end{itemize}
