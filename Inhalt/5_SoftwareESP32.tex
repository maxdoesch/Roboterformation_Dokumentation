...
Anschließend müssen die folgenden Schritte zur Einrichtung des Repositories und Programmierung des Roboters durchgeführt werden:

\begin{enumerate}
    \item GitHub Repository klonen \newline
    \verb|git clone https://github.com/maxdoesch/Roboterformation_ESP-IDF.git|
    ...
    \verb|export.bat| (Windows) oder \verb|source export.sh| (Unix)
    \item Chiptyp auf ESP32 einstellen \newline
    \verb|idf.py set-target esp32|
    \item Konfigurationsmenü starten \newline
    \verb|idf.py menuconfig|
    \begin{itemize}
        \item Unter \verb|Serial flasher config| die Einstellung \verb|Flash size| auf \verb|8MB| setzen
        \item Unter \verb|Component config/ESP32-specific| die Option \verb|Support for external, SPI-connected RAM| auswählen
        \item Unter \verb|Wifi Configuration| Wifi-SSID und Passowrt eingeben
        \item Unter \verb|Socket Configuration| die IP-Adresse des ROS-Bridge-Server Computers eingeben
    \end{itemize}
    \item Falls der Roboter im Simulationsmodus betrieben werden soll \verb|#define USE_SIM| in der main.cpp auskommentieren
    \item Jedem Roboter muss ein unterschiedlicher Name zugewiesen werden. Der Robotername wird in der main.cpp beim Aufruf von \verb|ros::NodeHandle::init()| festgelegt.
    \item Projekt bauen \newline
    \verb|idf.py build|
