...
Durch Inkludieren der Datei sdkconfig.h werden die Einstellungen in die C/C++ Programme eingebunden.
...
Dies erhöht die Struktur und Wiederverwendbarkeit des Codes. 
...
\begin{itemize}
    ...
    \item \emph{idf.py -C components create-component <Komponentenname>} Erstellt eine neue Komponente
\end{itemize}
...
\begin{itemize}
    ...
    \item \bverb|usStackDepth| Jeder Prozess hat einen eigenen Stack. Die Grö{\ss}e wird beim Erstellen der Task statisch festgelegt und muss somit vom Programmierer geeignet dimensioniert werden.
    Gro{\ss}e lokale Arrays oder viele verschachtelte Funktionsaufrufe im Prozess benötigen einen grö{\ss}eren Stack.
...
\begin{table}[H]
    ...
    Name                           & Komponente         & Klasse           & Priorität & \begin{tabular}[c]{@{}c@{}}Stackgrö{\ss}e\\ (Byte)\end{tabular} \\ \hline
    app\_main()                    & main               &                  & 0         &                                                             \\
...
an OutputVelocity weiter. OutputVelocity transformiert den Geschwindigkeitsvektor in Solldrehzahlen für die beiden Motoren. Diese werden dann im MotorController ausgeregelt.
Optional kann der Roboter auch in einen Simulationsmodus versetzt werden. Hierbei wird der Sollgeschwindigkeitsvektor an ROS gesendet. Au{\ss}erdem erhält die Komponente SensorPose
...
LedStrip steuert das Led-Array auf dem Roboter. Mithilfe von DataLogger können Programmlogs erstellt und in das ROS-System gesendet werden.
...
anschlie{\ss}end für bestimmte Wifi Events die Event Loop Library konfiguriert. Dabei handelt es sich um eine Zustandsmaschine, die bei Events wie z.B.
...
\section{RosBridgeClient} \label{ch:rosbridgeclient}
...
\begin{itemize}
    ...
    Anschlie{\ss}end werden die Daten deserialisiert und die enstprechende Callback-Funktion aufgerufen.
...
Aus diesem Grund müssen alle Nachrichtenklassen die folgenden Methoden implementieren:
...
\begin{itemize}
    ...
    \item \bverb|void deserialize(uint8_t* buffer)| wird von der Klasse SubscriberImpl aus RosBridgeClient zur Deserialisierung der Daten verwendet.
\end{itemize}


\section{LedStrip}
Die Klasse LedStrip ist für die Ansteuerung der LEDs zuständig. Es ist möglich Animationen abzuspielen oder die LEDs einheitlich in einer Farbe anzusteuern. 
Von der WebApp wird ein String an den ESP32 geschickt, welcher die Informationen über die Farbe und Animation enthält.

Aktuell sind folgende fünf Animationen vorhanden:

\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|}
    \hline
    Animation       & Erklärung     \\ \hline
    rainbow         & Alle LEDs ändern ihre Farbe im Regenbogenverlauf          \\ \hline
    pulse           & Alle LEDs ändern ihre Helligkeit von hell zu dunkel und anschließend von dunkel zu hell           \\ \hline
    line            & Die LEDs erleuchten von innen nach außen           \\ \hline
    circle          & Die LEDs wurden in drei Kreise eingeteilt. Diese erleuchten von innen nach außen           \\ \hline
    specific        & Alle LEDs leuchten in der von der WebApp eingestellten Farbe           \\ \hline
    \end{tabular}
    \caption{Animationen}
\end{table}


Ein Beispiel für einen gesendeten String würde so aussehen: \bverb |pulse,255,255,0| und enthält folgende Informationen: 
...
\begin{table}[H]
    ...
    \hline
    Animation   & Red   & Green & Blue   \\ \hline
    pulse       & 255   & 255   & 0      \\ \hline
    \end{tabular}
    \caption{Aufbau des Strings}
\end{table}



Die LEDs empfangen einen Bitstream, von welchem sich die einzelnen LEDs die ersten 24 Bits abschneiden und den verbleibenden Bitstream an die restlichen LEDs weiterführt.
In den 24 Bits sind die RGB Werte enthalten, 8 Bit je Farbe.

Im Folgenden wird auf die Methoden der Klasse LedStrip eingegangen:

\begin{itemize}
    \item \verb|LedStrip()| ist der Konstruktor der Klasse. Dort werden die Einstellungen für die LEDs, wie die Anzahl der LEDs und GPIO Pin, vorgenommen.
    \item \verb|void hsv2rgb(uint32_t h, uint32_t s, uint32_t v, uint32_t *r, uint32_t *g, uint32_t *b)| Diese Methode wird für manche Animationen benötigt. Hierbei wird das Farbspektrum von HSV in RGB Werte umgerechnet. Dies vereinfacht das erstellen von Animationen.
    \item \verb|void rgb2hue()| Um das Erstellen von Animationen zu vereinfachen, wurde diese Methode erstellt, um die RGB Werte in den Farbwinkel hue umzuwandeln.
    \item \verb|void animation_callback(void *arg)| ist die Callbackfunktion der LedStrip Klasse. Über die Node wird ein String empfangen, welcher anschließend die richtige Animation in der \verb|led_handler|Methode aufruft.
    \item Die \verb|void led_handler| Methode ist für das Abspielen der jeweiligen Animation zuständig. Die WebApp sendet einen string über die Callbackfunktion an die \verb|led_handler| Methode, welcher in die Einzelteile - Animation und RGB Werte - aufgeteilt wird. Mithilfe der Callback Funktion wird die richtige Animation aufgerufen. 
\end{itemize}
...
Das Stellsignal ist die Pulsbreite der Ausgangssignale für den Motortreiber. In der Software hat diese einen Wertebereich von -100.0 bis 100.0 Prozent.
...
\begin{itemize}
    ...
    \item \bverb|float updatePIControl(float actual_velocity)| aktualisiert den PI-Regler. Die Methode gibt die Pulsbreite des neuen PWM Stellsignals zurück. 
...
Dieser blockiert solange bis er aus der Interruptfunktion aufgeweckt wird. Dann holt sich der Prozess die aktuelle Drehzahl der Motoren \bverb|getActualVelocity()|, aktualisiert die PI-Regler der beiden Motoren\bverb|updatePIControl()|, setzt die neue Pulsbreite \bverb|setDuty()| und wartet,
...
dass das Interrupt möglichst schnell durchläuft und niemals blockiert wird. Zuletzt wird aus MotorController auch der Enable Pin für den Motortreiber aktiviert.
...
\begin{itemize}
    ...
    \item \bverb|void disablePIcontrol()| deaktiviert den PI-Regler. Wenn der PI-Regler deaktiviert ist, werden die Solldrehzahlen zur Steuerung der Pulsbreiten des PWM-Signals verwendet.
...
Ergibt die Transformation eine grö{\ss}ere Drehzahl, werden beide Drehzahlen um den gleichen Faktor runterskaliert.
\begin{itemize}
...
Die Klasse SensorPose definiert die gemeinsame Schnittstelle aller Sensoren und ermöglicht somit ein einfaches Wechseln zwischen unterschiedlichen Sensoren.
...
\begin{equation}
    \begin{gathered}
        ...
        \\
    \end{gathered}
    \label{eq:kalman_vorhersage}
\end{equation}

Im zweiten Update Schritt wird die Schätzung mithilfe der Positionsmessung $\tilde{y}_t$ von z.B. dem Marvelmindsensor korrigiert. Da jede Messung etwas Messrauschen $R$ besitzt, muss die Plausibilität der Messung im Vergleich zu der Schätzung bestimmt werden.
...
Des Weiteren ist es - durch Wiederholung des Update Schrittes für unterschiedliche Sensoren - möglich mehrere Messungen zu fusionieren.
Dies wird im Projekt über eine Liste von KalmanSensoren umgesetzt. Diese Liste wird in jedem Zyklus durchlaufen. Die Zustandsvariablen werden aktualisiert, falls neue Messungen verfügbar sind.
Die Berechnung der Vorhersage \ref{eq:kalman_vorhersage} befindet sich in einem FreeRTOS Prozess \bverb|_kalman_filter_loop_task()|, der über einen Softwaretimer auf $100Hz$ getaktet wird.
Der Update Schritt \ref{eq:kalman_update} ist in den Unterklassen von KalmanSensor implementiert und wird über den FreeRTOS Prozess aufgerufen. 
...
SensorPoseSim ist auch eine Unterklasse von SensorPose und KalmanSensor. Die Klasse dient zur Simulation des Roboters in ROS über z.B. den Turtlesim Simulator.
Die Roboterpose wird auf der Topic "`/robotername/pose"' empfangen. Diese wird im Konstruktor des SensorPoseSim Objektes abonniert. Die Methode \bverb|void _setPose()| dient dabei als Callbackfunktion für den RosBridgeClient Subscriber. 
...
Die Komponente PositionController setzt den äu{\ss}eren Regelkreise des Roboters um. Da der Roboter mit unterschiedlichen Regelaufgaben umgehen muss, ist kein Regler fest implementiert.
...
Anschlie{\ss}end wird ein neuer Geschwindigkeitsvektor als Stellgrö{\ss}e über das PositionController Objekt berechnet und dieser Vektor an das OutputVelocity Objekt weitergereicht.
...
\begin{itemize}
    ...
    Der Methode wird neben dem PositionController Objekt auch eine Callbackfunktion übergeben. Diese Callbackfunktion wird ausgeführt, wenn das Ende der Regelaufgabe erreicht wurde.
...
Die Regelung ist komplett innerhalb der Unterklassen dieser Schnittstelle gekapselt. ControllerMaster besitzt somit keine Information über die Art und das Ziel der Regelung.

\begin{itemize}
    \item Mit \bverb|ros_msgs_lw::Twist2D update(ros_msgs_lw::Pose2D const& actual_pose)| wird für eine neue Istposition ein neuer Geschwindigkeitsvektor als Stellgrö{\ss}e berechnet.
...
Über den Punkt zu Punkt Regler können einzelne Punkte angefahren werden. Der Zielpunkt wird dem Konstruktor bei der Erstellung des Reglerobjektes übergeben.
...
Eine Trajektorie besteht aus einem Array von Trajektorienpunkten.

Jeder Trajektorienpunkte beinhaltet Positions-, Geschwindigkeits-, Beschleunigungsvektoren und einen Zeitstempel.
...
Somit ist die Geschwindigkeit mit der die Trajektorie durchlaufen wird, unabhängig von der Aktualisierungsrate des Reglers.
...
Die Komponente StateMachine steuert das Verhalten des Roboters auf höchster Ebene. Eine Zustandsmaschine kontrolliert das Verhalten bei bestimmten Aktionen.
...
\section{DataLogger} \label{ch:datalogger_esp}
...
Die einzelnen Logs werden dann in einem Puffer zwischengespeichert, bis dieser eine bestimmte Grö{\ss}e erreicht hat. Anschlie{\ss}end wird der gesamte Puffer über den Nachrichtentyp \bverb|ros_msgs::String| 
...
\begin{itemize}
    ...
    \item \bverb|void _data_logger_task(void *pvParameters)| sendet den Puffer an das ROS-System, wenn dieser eine maximale Grö{\ss}e erreicht hat.
\end{itemize}
...
\begin{itemize}
    ...
    \item \bverb|#define USE_SIM| konfiguriert den Roboter im Simulationsmodus. SensorPoseSim dient somit als Quelle der Postionsdaten und die Geschwindigkeitdaten werden an OutputVelocitySim weitergegeben.
...
\begin{enumerate}
    ...
    \item Das Arbeitsterminal mit den Skripten aus dem Ordner "'esp/esp-idf"' initialisieren \newline
    ...
    \end{itemize}
    \item Falls der Roboter im Simulationsmodus betrieben werden soll, muss \bverb|#define USE_SIM| in der main.cpp auskommentiert werden.
