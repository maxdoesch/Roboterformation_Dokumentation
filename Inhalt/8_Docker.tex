...
Der gro{\ss}e Vorteil ist, dass man Container letztlich wie Rezepte zum Beschreiben verwendet, die ein leichtes Installieren der Software garantieren.\\
...
\begin{enumerate}
  \item Stellen Sie zunächst sicher, dass Ihr Windows auf dem neuesten Stand ist.
  \begin{itemize}
    \item Geben Sie in der Windows-Suche "'Windows Update"' ein und wählen Sie "'Windows Update-Einstellung"'.
    \item Wenn Sie ein grünes Häkchen sehen, sind  Sie auf dem neuesten Stand. Falls nicht, klicken Sie auf "'Nach Updates suchen"'. Sie müssen diesen Vorgang so lange wiederholen, bis Sie keine Updates mehr zu installieren haben.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    Maustaste auf "'Windows PowerShell"' und dann auf "'Als Administrator ausführen"'.
    ...
    \item Aktivieren Sie anschlie{\ss}end die Plattform für virtuelle Maschinen. Im Windows PowerShell ausführen. 
    (kopieren und einfügen) "'dism.exe /online /enable-feature /featurename:VirtualMachinePlatform /all /norestart"'.
    \item Laden Sie das \href{https://wslstorestorage.blob.core.windows.net/wslblob/wsl_update_x64.msi}{WSL2 Linux-Kernel-Update-Paket für x64-Maschinen} herunter und installieren Sie dies.
    ...
    \item Als nächstes installieren Sie eine Linux-Distribution aus dem \href{https://aka.ms/wslstore}{Microsoft Store}. Ich empfehle Ihnen \href{https://www.microsoft.com/store/productId/9MTTCL66CPXJ}{Ubuntu 20.04.4 LTS}, wobei das Herunterladen und Installieren einige Minuten andauern kann.
    ...
    \item Sie können nun Linux-Befehle im Ubuntu-Terminalfenster ausführen. Ich empfehle Ihnen, das Ubuntu-Symbol an die Taskleiste zu heften.
  \end{itemize}
  ...
  \begin{itemize}
  \item Führen Sie das Installationsprogramm aus und starten Sie anschlie{\ss}end Windows neu.
  \item Melden Sie sich bei Windows an und starten Sie Docker-Desktop. Lassen Sie Docker die Einrichtung abschlie{\ss}en, dies kann je nach Rechner einige Minuten dauern.
\end{itemize}
\end{enumerate}
\newpage
\section{Unsere Verwendung von Docker}
Die Einleitung zum Einrichten des Docker-Compose finden Sie in der \texttt{README.md} Datei von dem \href{https://git.efi.th-nuernberg.de/gitea/doeschma79099/Roboterformation_Docker}{Docker-Git-Repository}.
...
\textit{docker-compose.yml} Datei verwenden, ausführlich erklärt. 
...
networks: #Mit diesen Befehlen erzeugen wir ein Netzwerk namens "rosnet"
  rosnet:

services: #Unter services werden die einzelnen Container definiert
  ...
  master: #Hier wird ein Container mit dem Namen "master" angelegt
    ...
    command:
      - roscore #Mit command wird der Befehl "roscore" in der Kommandozeile des Containers ausgefuehrt
    networks:
      - rosnet #networks definiert das Netzwerk des masters 

#Da, die Einstellungen und Befehle von den letzten zwei Container sich aehneln, wird es nur fuer einen Container erklaert.  
  rosbridge: #Hier wird ein Container mit dem Namen "rosbridge" angelegt
    build: 
      context: ./rosbridge #Als image, bzw build wird hier zu einem Dockerfile navigiert, welches sich unter dem Verzeichnis "./rosbridge" befindet
    environment:
      - "ROS_HOSTNAME=rosbridge"
      - "ROS_MASTER_URI=http://master:11311" #Unter environment werden Environmentvariablen festgelegt, in diesem Fall, wie der HOSTNAME des Containers lautet und wo sich der master im Netzwerk befindet
    networks:
      - rosnet 
    depends_on: 
      - master #Der Container faehrt erst hoch, wenn der master bereits laeuft und schaltet sich vor dem master aus 
    ports:
      - 9090:9090 #Hier werden Ports in dieser Form (HOST-PORT:CONTAINER-PORT) nach aussen freigegeben 
...
Im Unterkapitel \hyperref[dockercompose]{Docker-Compose} werden in einer \textit{docker-compose.yml} Datei mehrere \hyperref[buildBefehle]{build-Befehle} aufgerufen, welche zu dem eigentlichen \textit{Dockerfile} nagivieren und diesen als Hauptimage bauen.
Im Folgenden werden die Befehle aus dem \textit{Dockerfile} und deren Funktionen anhand eines Beispiels aus unserem Projekt erklärt und geschildert. 
...
\begin{lstlisting}[language=Dockerfile,caption={Dockerfile (rosbridge)},breaklines=true,basicstyle=\footnotesize]
  ...
  RUN apt-get -y upgrade # -y sorgt dafuer, dass wenn nach dem Ausfuehren des Befehls eine JA-NEIN-Frage erscheinen soll, diese automatisch mit JA bzw. YES beantwortet wird
  RUN apt-get -y install ros-noetic-rosbridge-server 
  COPY ./start.sh start.sh #COPY-Befehl kopiert die Datei start.h auf unser Image. COPY SOURCE-Verzeichnis DESTINATTION-Verzeichnis
